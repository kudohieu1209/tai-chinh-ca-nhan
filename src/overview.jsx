...
function Overview({ transactions, allTransactions, debts, budgets = [], goals, viewMonth, viewYear, monthLabel, openingBalance = 0, periodBalance, closingBalance, onNavigate, onSaveGoal, onDeleteGoal }) {
  ...
  const reportAllTransactions = useMemo(() => {
    return allTransactions.filter(isVisibleDebtPayment);
  }, [allTransactions, isVisibleDebtPayment]);

  const income = useMemo(() =>
    reportTransactions.filter(t => t.type === "income").reduce((s, t) => s + t.amount, 0),
    [reportTransactions]
  );
  const expense = useMemo(() =>
    reportTransactions.filter(t => t.type === "expense").reduce((s, t) => s + t.amount, 0),
    [reportTransactions]
  );
  ...
  const sixMonths = useMemo(() => {
    return Array.from({ length: 6 }, (_, i) => {
      ...
      const txns = reportAllTransactions.filter(t => {
  ...
  }, [reportAllTransactions, viewMonth, viewYear]);
  ...
  const fixedPaceExpense = reportTransactions
  ...
  const budgetWatch = budgets
    .map(b => {
      const actual = reportTransactions
