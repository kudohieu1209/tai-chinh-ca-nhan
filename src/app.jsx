  ...
  useEffect(() => {
    ...
    const unsub = window._fsDoc.onSnapshot(async snap => {
      if (snap.exists) {
        ...
        // First login after auth rollout: copy the old shared document into the
        // user's private document before falling back to local backup/sample data.
        const legacySnap = await db.collection("fintrack").doc("hiewu").get().catch(err => {
          console.warn("Legacy data migration skipped:", err);
          return null;
        });
        const local = readLocalData();
        const fallback = legacySnap?.exists ? legacySnap.data() : local || seedData();
        setDataSource(legacySnap?.exists ? "remote" : local ? "local" : "seed");
        ...
        window._fsDoc.set({
          ...
          migratedFrom: legacySnap?.exists ? "fintrack/hiewu" : local ? "localStorage" : "seed",
