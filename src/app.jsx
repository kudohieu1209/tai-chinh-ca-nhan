...
function authErrorMessage(err, tr = (vi) => vi) {
  const code = err?.code || "";
  if (code.includes("invalid-email")) return tr("Email chưa đúng định dạng.", "That email address looks invalid.");
  if (code.includes("user-not-found") || code.includes("wrong-password") || code.includes("invalid-credential")) {
    return tr("Email hoặc mật khẩu chưa đúng.", "Wrong email or password.");
  }
  if (code.includes("email-already-in-use")) return tr("Email này đã có tài khoản rồi.", "This email already has an account.");
  if (code.includes("weak-password")) return tr("Mật khẩu nên có ít nhất 6 ký tự.", "Password should be at least 6 characters.");
  if (code.includes("popup-closed")) return tr("Bạn đã đóng cửa sổ đăng nhập.", "You closed the sign-in window.");
  if (code.includes("operation-not-allowed")) return tr("Provider đăng nhập này chưa được bật trong Firebase.", "This sign-in provider isn't enabled in Firebase.");
  return err?.message || tr("Không đăng nhập được, thử lại giúp mình.", "Couldn't sign in, please try again.");
}

function AuthGate({ theme, onTheme, lang, onLang }) {
  const t = useT();
  ...
  const resetPassword = async () => {
    if (!email.trim()) {
      setError(t("Nhập email trước rồi mình gửi link đặt lại mật khẩu.", "Enter your email first, then we'll send a reset link."));
      return;
    }
    setBusy(true);
    setError("");
    try {
      ...
      setError(t("Đã gửi email đặt lại mật khẩu nếu tài khoản tồn tại.", "A password reset email has been sent if the account exists."));
    } catch (err) {
      setError(authErrorMessage(err, t));
    } finally {
      setBusy(false);
      ...
      <section className="auth-card">
        <div className="auth-head">
          <div>
            ...
            <h1>{isSignup ? t("Tạo tài khoản", "Create account") : t("Đăng nhập", "Sign in")}</h1>
            <p>{isSignup ? t("Dữ liệu sẽ được lưu theo tài khoản riêng của bạn.", "Your data is saved to your own private account.") : t("Quản lý chi tiêu, ngân sách và dữ liệu tài chính của bạn.", "Manage your spending, budgets and financial data.")}</p>
          </div>
          {onLang && (
            <div className="settings-row" style={{ display: "flex", gap: 6 }}>
              <button type="button" className={"settings-opt-btn" + (lang === "vi" ? " active" : "")} onClick={() => onLang("vi")}>VN</button>
              <button type="button" className={"settings-opt-btn" + (lang === "en" ? " active" : "")} onClick={() => onLang("en")}>EN</button>
            </div>
          )}
        </div>
        ...
        <form className="auth-form" onSubmit={submitEmail}>
          {isSignup && (
            <label className="field">
              <span className="field-label">{t("Tên hiển thị", "Display name")}</span>
              <input className="input" value={displayName} onChange={e => setDisplayName(e.target.value)} autoComplete="name" placeholder={t("Hiếu", "Your name")} />
            </label>
          )}
          <label className="field">
          ...
          <label className="field">
            <span className="field-label">{t("Mật khẩu", "Password")}</span>
            <div className="input-password">
              <input className="input" type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} autoComplete={isSignup ? "new-password" : "current-password"} placeholder={t("Tối thiểu 6 ký tự", "At least 6 characters")} required />
              <button
                type="button"
                className="input-password-toggle"
                onClick={() => setShowPassword(v => !v)}
                aria-label={showPassword ? t("Ẩn mật khẩu", "Hide password") : t("Hiện mật khẩu", "Show password")}
                title={showPassword ? t("Ẩn mật khẩu", "Hide password") : t("Hiện mật khẩu", "Show password")}
                tabIndex={-1}
              >
          ...
          <button className="btn auth-submit" type="submit" disabled={busy}>
            {busy ? t("Đang xử lý...", "Working...") : isSignup ? t("Tạo tài khoản", "Create account") : t("Đăng nhập", "Sign in")}
          </button>
        </form>
        ...
        <div className="auth-foot">
          <button type="button" onClick={() => { setMode(isSignup ? "signin" : "signup"); setError(""); }}>
            {isSignup ? t("Đã có tài khoản? Đăng nhập", "Already have an account? Sign in") : t("Tạo mới", "Create one")}
          </button>
          {!isSignup && <button type="button" onClick={resetPassword}>{t("Quên mật khẩu", "Forgot password")}</button>}
        </div>
      </section>
    </div>
  );
...
function App() {
  ...
  useEffect(() => {
    ...
    if (CATEGORIES["Ăn uống"]) {
      const newColor = theme === "codex" ? "#41ff8b" : "#FF9500";
      if (CATEGORIES["Ăn uống"].color !== newColor) {
        CATEGORIES["Ăn uống"].color = newColor;
  ...
  if (!authUser) return (
    <LangContext.Provider value={lang}>
      <AuthGate theme={theme} onTheme={setTheme} lang={lang} onLang={setLang} />
  ...
  const userLabel = authUser.displayName || authUser.email || t("Tài khoản", "Account");
  const userInitial = (userLabel.trim()[0] || "U").toUpperCase();

  const isOwner = isOwnerUser(authUser);
  const PageComponents = { overview: Overview, transactions: Transactions, debts: Debts, budget: Budget, notes: Notes, settings: Settings };
            ...
            <span>
              {dataSource === "seed"
                ? t(
                    ...
                    "Couldn't connect to the database — showing sample data. DON'T add/edit transactions right now. Your real data is still safe on the server; check the connection/Firestore and reload."
                  )
                : t(
                    "Chưa kết nối được cơ sở dữ liệu — đang hiển thị bản sao lưu cục bộ. Thay đổi sẽ không được đồng bộ cho tới khi kết nối lại.",
                    "Couldn't connect to the database — showing a local backup. Changes won't sync until the connection is restored."
                  )}
            </span>
          </div>
        )}
...
function BottomNav({ activePage, onChange, debts = [], lang = "vi" }) {
  ...
  const items = [
    ...
    { id: "debts",        label: lang === "en" ? "Debts"        : "Nợ vay",    icon: "creditCard" },
    { id: "budget",       label: lang === "en" ? "Budget"       : "Ngân sách", icon: "wallet" },
    { id: "settings",     label: lang === "en" ? "Settings"     : "Cài đặt",   icon: "gear" },
  ];
  return (
