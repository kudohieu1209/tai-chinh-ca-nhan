...
function Toolbar({ activePage, month, onMonthChange, closingBalance = 0, viewMonth, viewYear, theme, onTheme, lang, onLang }) {
  ...
  const hideToolbar = activePage === "overview" || activePage === "transactions";
          ...
          {settingsOpen && (
            <div className="settings-dropdown">
              ...
              <div className="settings-row">
                ...
                <button
                  className={"settings-opt-btn" + (theme === "codex" ? " active" : "")}
                  onClick={() => onTheme("codex")}
                >
                  <Icons.squareGrid size={13} /> Codex
                </button>
              </div>
      ...
      <div className="modal" style={{ maxWidth: width }} role="dialog" aria-modal="true" aria-label={title}>
        <div className="modal-head">
          ...
          <div className="modal-head-tools">
            {headerExtra}
            <button className="modal-close" onClick={onClose} aria-label={t("Đóng", "Close")} title={t("Đóng", "Close")}>
              <Icons.x size={15} />
            </button>
          </div>
        </div>
